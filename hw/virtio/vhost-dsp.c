...
static void vhost_dsp_handle_output(VirtIODevice *vdev, VirtQueue *vq)
{
    /* Do nothing */
}
...
static void vhost_dsp_device_realize(DeviceState *dev, Error **errp)
{
    ...
    virtio_init(vdev, "vhost-dsp", VIRTIO_ID_DSP,
                0/*sizeof(struct virtio_dsp_config)*/);

    /* All queues belong to the vhost */
    virtio_add_queue(vdev, VHOST_DSP_QUEUE_SIZE, vhost_dsp_handle_output);
    virtio_add_queue(vdev, VHOST_DSP_QUEUE_SIZE, vhost_dsp_handle_output);
    virtio_add_queue(vdev, VHOST_DSP_QUEUE_SIZE, vhost_dsp_handle_output);
...
static int vhost_dsp_start_ioeventfd(VirtIODevice *vdev)
{
    return 0;
}

static void vhost_dsp_stop_ioeventfd(VirtIODevice *vdev)
{
}
...
static int vhost_dsp_pre_save(void *opaque)
{
    return 0;
}

static int vhost_dsp_post_load(void *opaque, int version_id)
{
    /* VSock sets up a timer here, we might not need this function at all */
    return 0;
}
...
static const VMStateDescription vmstate_virtio_vhost_dsp = {
    ...
    .pre_save = vhost_dsp_pre_save,
    .post_load = vhost_dsp_post_load,
};
...
static void vhost_dsp_set_config(VirtIODevice *vdev, const uint8_t *config)
{
}
...
static void vhost_dsp_class_init(ObjectClass *klass, void *data)
{
    ...
    vdc->start_ioeventfd = vhost_dsp_start_ioeventfd;
    vdc->stop_ioeventfd = vhost_dsp_stop_ioeventfd;
    vdc->guest_notifier_mask = vhost_dsp_guest_notifier_mask;
    vdc->guest_notifier_pending = vhost_dsp_guest_notifier_pending;
    vdc->set_config = vhost_dsp_set_config;
}
